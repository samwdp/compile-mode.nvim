...
---
---@param opts { fname: string } | { bufnr: integer } | { swap_on: boolean }
---@param smods SMods
---@param count integer
...
local function jump_to_file(filename, error, smods)
    ...
    if config.use_diagnostics then
        ...
        compilation_buffer = M.split_unless_open(
        { bufnr = compilation_buffer, fname = config.buffer_name, swap_on = config.disable_swap }, smods, 0)
